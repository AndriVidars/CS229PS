...
class QP:
    ...
    def train_GD(self, X, Y, eta = 8e-2, max_step = 1000, 
        ...
        for t in range(max_step):
            ...
            theta_g, phi_g = self.gradient(X, Y)

            self.theta = self.theta - eta*theta_g
            self.phi = self.phi - eta*phi_g
            # *** END CODE HERE ***
            if verbose:
                log_steps.append(t)
    ...
    def train_SGD(self, X, Y, eta = 8e-2, max_step = 1000, batch_size = 1, 
        ...
        for t in range(max_step):
            ...
            # and update self.theta, self.phi
            theta_g, phi_g = self.gradient(X_batch, Y_batch)
            self.theta = self.theta - eta*theta_g
            self.phi = self.phi - eta*phi_g
            # *** END CODE HERE ***
            if verbose:
    ...
    def gradient(self, X, Y):
        ...
        """
        # *** START CODE HERE ***
        theta_g = (1/X.shape[0])*sum([(((X[i, :] @ (self.theta**2 - self.phi**2)) - Y[i]))*np.multiply(self.theta, X[i, :].T) for i in range(X.shape[0])])
        phi_g = -(1/X.shape[0])*sum([(((X[i, :] @ (self.theta**2 - self.phi**2)) - Y[i]))*np.multiply(self.phi, X[i, :].T) for i in range(X.shape[0])])

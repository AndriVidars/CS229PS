...
def linear_model_main():
    ...
    beta_0 = X.T @ np.linalg.solve(X @ X.T, Y)
    # *** END CODE HERE ***
    ...
    betas = [beta_0] + [beta_0 + ns[i, :] for i in np.random.randint(low = 0, high=ns.shape[0], size = 3, )]
    generate_plot(betas, X, Y, X_test, Y_test, save_path_linear)
    # *** END CODE HERE ***
